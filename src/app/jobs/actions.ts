...
export async function loadMoreJobsAction(offset: number, sortBy: MatchSortBy): Promise<JobRow[]> {
  const profileId = await getActiveProfileId();
  const [rows, applicationIdsByJob] = await Promise.all([
    matchingService.listRecent(profileId, JOBS_PAGE_SIZE, sortBy, offset),
